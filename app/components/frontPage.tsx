    ...
    <>
      ...
      <div className="w-full p-4 flex flex-col gap-2">
        {matches.map(({ winnerId, matchCreatedAt, matchId, eloDelta, players: [player1, player2] }) => {
          ...
          return <Card className="bg-white" key={matchId}>
            <CardHeader>
              <CardTitle className="text-center">{timeString}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-[1fr_32px_2px_32px_1fr]">
              <MatchParticipant {...player1} winner={winnerId === player1.id} className="justify-self-start" />
              ...
              <MatchParticipant {...player2} winner={winnerId === player2.id} className="justify-self-end" />
            </CardContent>
          </Card>
        })}
      </div>
